...
//----------------------------------------------------------------------------//
int main() {
    PUTS_("D3D12/SYCL async fence-signal hang reproducer");
    PUTS_("----------------------------------------------");
    ...
    ThrowIfFailed(d3dCtx.device->CreateFence(0, D3D12_FENCE_FLAG_NONE,
    ...
    //     syclexp::import_external_semaphore(semDesc, device, contxt);
    ...
    const uint64_t N = 1;
    uint64_t d3dSignalVal = N;
    uint64_t syclSignalVal = N + 1;
    ...
    ThrowIfFailed(d3dCtx.cmdQueue->Signal(extFence.fence.Get(), d3dSignalVal),
    ...
    PRINT_("[D3D12] Current external fence value = {}...\n", extFence.fence->GetCompletedValue());
    ThrowIfFailed(d3dCtx.cmdQueue->Wait(extFence.fence.Get(), syclSignalVal),
    ...
    // q.wait(); // Ensure the signal is submitted, but do NOT wait for it to complete

    // Pause the current thread for exactly 4 microseconds
    PUTS_("\nPausing for 1 microsecond before issuing GPU Wait(N+1)...");
    std::this_thread::sleep_for(std::chrono::microseconds(1));

    // Signal(N+2): simulates post-denoise tonemap/readback work.
    // Will never execute if Wait(N+1) is never satisfied.
    PRINT_("[D3D12] cmdQueue->Signal(extFence, {})\n", N + 2);
    ThrowIfFailed(d3dCtx.cmdQueue->Signal(extFence.fence.Get(), N + 2),
    ...
    PRINT_("[D3D12] CPU: SetEventOnCompletion({}) + "
           ...
           N + 2, kWaitTimeoutMs);
    ThrowIfFailed(extFence.fence->SetEventOnCompletion(N + 2, cpuFenceEvent),
                  "SetEventOnCompletion(N+2) failed");

    uint64_t curFenceVal = extFence.fence->GetCompletedValue();
    ...
    if (waitResult == WAIT_TIMEOUT) {
        PRINT_("\n"
               "[FAIL] TIMEOUT -- fence never reached {}.\n"
               "       Fence's completed value = {}\n"
               "       The D3D12 GPU queue is stuck at Wait(extFence, {})\n"
               "       SYCL's async ext_oneapi_signal_external_semaphore({}) did not\n"
               "       advance the D3D12 fence, so the GPU-side Wait was never satisfied.\n",
               N + 2, curFenceVal, N + 1, N + 1);
    ...
    PRINT_("[PASS] Fence reached {} -- SYCL async signal successfully unblocked the D3D12 GPU queue.\n", N + 2);
